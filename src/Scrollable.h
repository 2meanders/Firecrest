...
public:
    Scrollable(alignment::ElementAlignment alignment, ShapeRenderer2D& renderer)
        : VerticalPositioning(alignment, VerticalPositioning::Direction::TopToBottom),
          _flexElement(createChild<Element>(
              alignment::ElementAlignment().setHeight(alignment::Pixels(0.0f)))),
          _renderer(renderer) {}
    ...
    virtual void render(const Window& window, time::Duration delta) override {
        ...
        if (clampScroll) {
            ...
            const float height
                = std::clamp(visualHeight * (visualHeight / totalHeight), 30.0f, visualHeight);
            const float minOffset = std::min(0.0f, elSize.y - totalHeight);
            const float y
                = maths::map(_verticalScrollOffset, minOffset, 0.0f, 0.0f, visualHeight - height);
            _renderer.roundedRect(window, elPos + glm::vec2(elSize.x - width - padding, y),
                                  {width, height}, {0.5, 0.5, 0.5, 0.75}, width * 0.5f, 12);
        }
    ...
    void scroll(float pixels) {
        ...
        } else if (!_stickingToBottom
                   && getPixelSize().y - contentHeight() >= _verticalScrollOffset) {
    ...
    // Scrolls to the top
    void goToTop() { _verticalScrollOffset = 0.0f; }
    ...
    float getScrollOffset() const { return _verticalScrollOffset; }
};
} // namespace fc