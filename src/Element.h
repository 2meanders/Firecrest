...
protected:
    // Lock the mouse to this element, so that it will not move outside of this
    // element. Returns true if the mouse was successfully locked, false
    // otherwise. Note: only the focused element can lock the mouse.
    bool lockMouse();
...
public:
    ...
    // so there is no need to check if the mouse position is within the
    // element's rectangle.
...
private:
    ...
    virtual int32_t maxDepth() const { return calculateDepth(); }
