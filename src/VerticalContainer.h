...
public:
    void setFlexibleChildren(std::vector<fiv::ID> flexibleChildren) {
        alignment::AlignmentFunction alignment([flexibleChildren, this](float parent1,
                                                                        float parent2) {
            float totalSize = 0.0f;
            size_t index = 0;
            for (int i = 0; i < children().size(); i++) {
                const auto& child = children().dataAt(i);
                if (std::find(flexibleChildren.begin(), flexibleChildren.end(), children().idAt(i))
                    != flexibleChildren.end()) {
                    index++;
                    continue;
                }
