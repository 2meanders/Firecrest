...
public:
    Display(Window& window)
        ...
        _keySubscription = _window.getInput().subscripeKeyEvent(
            [this](input::RawKeyboardEvent event) { keyCallback(event); });
        _charSubscription = _window.getInput().subscribeCharEvent(
            [this](input::UnicodeCodePoint letter) { charCallback(letter); });
        _mouseMotionSubscription = _window.getInput().subscribeMouseMotionEvent(
            [this](input::RawMouseMotionEvent event) { mouseMotionCallback(event); });
        _mouseButtonSubscription = _window.getInput().subscribeMouseButtonEvent(
            [this](input::RawMouseButtonEvent event) { mouseButtonCallback(event); });
        _scrollSubscription = _window.getInput().subscribeScrollEvent(
            [this](input::RawScrollEvent event) { scrollCallback(event); });
    }
    ...
    virtual int32_t calculateDepth() const override { return 0; }
    ...
    void mouseButtonCallback(input::RawMouseButtonEvent event) {
        ...
        if (!mouseIsLocked) {
            for (auto& child : children()) {
                ...
                input::MouseButtonEvent e{static_cast<input::MouseButton>(event.button),
            ...
            // If the mouse is locked, we only send the event to the focused
            // element
            input::MouseButtonEvent e{static_cast<input::MouseButton>(event.button),
                                      static_cast<input::MouseButtonAction>(event.action),
                                      event.mods};
        ...
        if (event.button == input::RawMouseButton::Left
            && event.action == input::RawMouseButtonAction::Up) {
        ...
        if (!mouseIsLocked) {
            for (auto& child : children()) {
                ...
                if (wasInside && isInside)
                    child->onMouseMotionEvent(
                        _window.getInput(),
                        {input::MouseMotionAction::Move, event.position, event.lastPosition});
                else if (wasInside && !isInside)
                    child->onMouseMotionEvent(
                        _window.getInput(),
                        {input::MouseMotionAction::Exit, event.position, event.lastPosition});
                else if (!wasInside && isInside)
                    child->onMouseMotionEvent(
                        _window.getInput(),
                        {input::MouseMotionAction::Enter, event.position, event.lastPosition});
            }
        } else if (_focusedElement != nullptr) {
            // If the mouse is locked, we only send the event to the focused
            // element
            _focusedElement->onMouseMotionEvent(
                _window.getInput(),
                {input::MouseMotionAction::Move, event.position, event.lastPosition});
        }
    }
...
public:
    virtual bool lockMouse(Element* requestingElement) override {
        if (_focusedElement == nullptr || _focusedElement != requestingElement
            || _window.isMouseLocked()) {
            return false;
        }
        _window.lockMouse();
        return true;
