...
public:
    ...
    inline const std::vector<input::UnicodeCodePoint>& charactersTyped() const {
        return _typedCharacters;
    }

    [[nodiscard]]
    fiv::ID subscripeKeyEvent(KeyEventCallback callback) {
        return _keyEventCallbacks.push(callback);
    }
    void unsubscribeKeyEvent(fiv::ID id) { _keyEventCallbacks.remove(id); }

    [[nodiscard]]
    fiv::ID subscribeMouseMotionEvent(MouseMotionEventCallback callback) {
        return _mouseMotionEventCallbacks.push(callback);
    }
    void unsubscribeMouseMotionEvent(fiv::ID id) { _mouseMotionEventCallbacks.remove(id); }

    [[nodiscard]]
    fiv::ID subscribeMouseButtonEvent(MouseButtonEventCallback callback) {
        return _mouseButtonEventCallbacks.push(callback);
    }
    void unsubscribeMouseButtonEvent(fiv::ID id) { _mouseButtonEventCallbacks.remove(id); }

    [[nodiscard]]
    fiv::ID subscribeCharEvent(CharEventCallback callback) {
        return _charEventCallbacks.push(callback);
    }
    void unsubscribeCharEvent(fiv::ID id) { _charEventCallbacks.remove(id); }

    [[nodiscard]]
    fiv::ID subscribeScrollEvent(ScrollCallback callback) {
        return _scrollEventCallbacks.push(callback);
    }
