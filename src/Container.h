...
public:
    ...
    template <typename T, typename... Args> T& createChild(Args&&... args) {
    ...
    template <typename T, typename... Args> std::pair<T&, fiv::ID> createIDedChild(Args&&... args) {
        static_assert(std::is_base_of<Element, T>::value, "T must derive from Element");
        fiv::ID id = _children.emplace(std::make_unique<T>(std::forward<Args>(args)...));
        _children[id]->_parent = this;
        childCreated(id);
    ...
    const fiv::Vector<std::unique_ptr<Element>>& children() const { return _children; }
        ...
        for (auto& child : children()) {
            ...
            if (wasInside && isInside)
                child->onMouseMotionEvent(
                    input, {input::MouseMotionAction::Move, event.position, event.lastPosition});
            else if (wasInside && !isInside)
                child->onMouseMotionEvent(
                    input, {input::MouseMotionAction::Exit, event.position, event.lastPosition});
            else if (!wasInside && isInside)
                child->onMouseMotionEvent(
                    input, {input::MouseMotionAction::Enter, event.position, event.lastPosition});
        }
    }
    ...
    virtual glm::vec2
    calculateChildPixelPosition(const alignment::ElementAlignment& childAlignment) const {
