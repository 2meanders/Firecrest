...
public:
    ...
    explicit Tracked(const T& initialValue, bool hasChanged)
        : _currentValue(initialValue), _hasChanged(hasChanged) {}
    ...
    operator const T&() const { return _currentValue; }

    // Comparison operators
    bool operator==(const T& other) const { return _currentValue == other; }
    bool operator!=(const T& other) const { return _currentValue != other; }

    /// @brief Checks whether the value has been modified since the last call to
    /// `isModified`.
    bool isModified() {
    ...
    void markModified() { _hasChanged = true; }
};
